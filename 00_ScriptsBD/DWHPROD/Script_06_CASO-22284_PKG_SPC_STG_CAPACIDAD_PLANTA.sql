CREATE OR REPLACE PACKAGE DWHADM.PKG_SPC_STG_CAPACIDAD_PLANTA
------------------------------------------------------------------------------------------------------------
-- RESUMEN
...
-- Descripción : Paquete para ejecutar el esquema STG del Proyecto
------------------------------------------------------------------------------------------------------------
-- Modificaciones
-- Requerimiento Responsable Fecha Descripción
------------------------------------------------------------------------------------------------------------
 IS
  -- Variables utilizadas en el proceso de Carga para el rango de fechas
  ...
  ------------------------------------------------------------------------------------------------------------
  -- RESUMEN
  -- Descripción : Carga de Fuerza Laboral, Vacaciones y Organico en el esquema STG
  ...
  ---------------------------------------------------------------------------------------------------------------------------------
  -- RESUMEN
  -- Descripción : Carga de presolicitud de prestamos en las oficinas edyficar en el esquema STG
  -- Fecha de Creación : 08/07/2014
  -- Autor : Diego Zegarra T.
  -- Tabla Destino : STG.T_PRESOLICITUD
  ...
  ---------------------------------------------------------------------------------------------------------------------------------
  -- RESUMEN
  -- Descripción : Carga de cobros o recaudaciones realizadas en el esquema STG
  -- Fecha de Creación : 08/07/2014
  -- Autor : Diego Zegarra T.
 ...
 ---------------------------------------------------------------------------------------------------------------------------------
  ...
  ------------------------------------------------------------------------------------------------------------
  -- RESUMEN
  -- Descripción : Carga de desembolsos de prestamos realizados en el esquema STG
...
--@006 PRZ CASO-22284: Se agregam 4 campos en el select para llenar la tabla STG.T_DESEMBOLSO (COD_ANA - COD_PRO - VL_TCEA - NU_PLA_DIAS)
------------------------------------------------------------------------------------------------------------
 IS
  ------------------------------------------------------------------------------------------------------------
  -- RESUMEN
   ...
   EXECUTE IMMEDIATE 'ALTER SESSION SET NLS_DATE_FORMAT = ''DD/MM/YYYY''';
    ...
    INTO STG.T_DESEMBOLSO
      ( NU_PERI_MES,
        ...
        CO_USUA_TOPA_DESE,
        --@002 Fin
		--@006 ini: Se agregan 4 campos nuevos a la tabla STG.T_DESEMBOLSO
        COD_ANA,
        COD_PRO,
        VL_TCEA,
        NU_PLA_DIAS
      ...
      SELECT V_PERIODO AS NUM_PERI_MES,
			...
			--@004 Fin
			--@006 Ini: Se agregan 4 campos en el select
            C.C1670 AS COD_ANA, 
            C.PRODUCTO AS COD_PRO,
            D.C5039 AS VL_TCEA,
            D.C5223 AS NU_PLAZ_DIAS
            --@006 Fin
       ...
       INNER JOIN SL_SOLICITUDCREDITO  D
         ...
         AND C.C9314 = 5
         AND C.OPERACION = 0
		  --@005 Ini: Se cambia el filtro del campo C.C1621 por el C.1620
